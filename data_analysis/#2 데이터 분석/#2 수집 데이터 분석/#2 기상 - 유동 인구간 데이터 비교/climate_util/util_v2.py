...
def import_station_num(target):
    loc_df = pd.read_csv('./climate_util/인근 관측소.csv', index_col=0)
    stn = loc_df.loc[loc_df['Area name']==target, 'Station Code'].values[0]
    return stn

def analyze_population_data(target, start_year, option):  # ex) 군자역
    stn = import_station_num(target)
    api_path = './climate_util/climate_api_key.txt'
    with open(api_path, 'r') as f:
        api_key = f.readline().strip()
    tm1 = f"{start_year}01010000"
    tm2 = f""
    df_climate = import_climate_data(stn, tm1, tm2, api_key)
    df_climate = import_climate_data(stn, tm1, tm2, api_key)
    df_pop = pd.read_csv(f'./{target}_생활인구수.csv', index_col=0)
    df_pop.index = pd.to_datetime(df_pop.index)
    df_total = df_climate.join(df_pop, how='left')
    df_total = df_total.loc[df_total.index.year != 2025]
    ...
    # 시각화 함수들
    # plot_yearly_gender_distribution(df_total)       # 연도별 평균 성별 분포
    # plot_yearly_age_distribution(df_total)            # 연도별 평균 나이대 분포
    plot_correlation_heatmap(df_total)                # 상관관계 히트맵
    # plot_daily_time_series(df_total, target)                  # 일별 시계열 분석
    # plot_monthly_time_series(df_total, target)                # 월별 시계열 분석
    # plot_demographic_population_pattern(df_total)     # 요일/시간대/나이 및 성별 별 인구 패턴
    
    return df_total
    
def plot_yearly_gender_distribution(df_total):
    # 성별 데이터 생성
...
def plot_yearly_age_distribution(df_total):
...
def plot_correlation_heatmap(df_total):
    age_columns = ['10gen_male', '10gen_female', '20gen_male', '20gen_female', 
                '30gen_male', '30gen_female', '40gen_male', '40gen_female', 
                '50gen_male', '50gen_female', '60gen_male', '60gen_female']
    
    correlation = df_total[['총생활인구수'] + age_columns + ['기온', '풍속', '1시간 강수량', '습도']].corr()
    input_data = correlation[['기온', '풍속', '1시간 강수량', '습도']].loc[['총생활인구수'] + age_columns]
    plt.figure(figsize=(5, 7))
    sns.heatmap(input_data, annot=True, cmap='coolwarm', vmin=-1, vmax=1, center=0)
...
def plot_monthly_time_series(df_total, target):
    ...
    monthly_total = monthly_total.interpolate(method='linear')
    plt.figure(figsize=(14, 6))
...
def plot_demographic_population_pattern(df_total):
    # 요일과 시간 컬럼 추가
